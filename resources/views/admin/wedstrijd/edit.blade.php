    ...
    <div class="py-3 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="p-6 text-gray-900">
                <form action="{{ route('wedstrijd.update', $wedstrijd) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="mb-4">
                        <label for="naam" class="block text-sm font-medium text-gray-700">Naam</label>
                        <input type="text" name="naam" id="naam" value="{{ old('naam', $wedstrijd->naam) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="categorie_id" id="categorie_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @selected($category->id == $wedstrijd->categorie_id)>{{ $category->type }}</option>
                            @endforeach
                        </select>
                    </div> 
                    ...
                    <div class="mb-4">
                        ...
                        <input type="checkbox" name="zichtbaarheid" id="zichtbaarheid" value="1" class="mt-1" @checked($wedstrijd->zichtbaarheid)>
                    </div>
                    ...
                    <button type="submit" class="inline-block bg-green-500 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition">+ Update Wedstrijd</button>
                </form>
            </div>
        </div>
    </div>
